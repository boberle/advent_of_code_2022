#include <stdio.h>
#include <stdlib.h>
#include <string.h>


#define BUF_LEN 5

enum result {win, loss, draw};


int get_score(FILE *);
char standardize_move(char);
enum result does_b_win(char, char);


int main(int argc, char **argv) {

   if (argc != 2) {
      printf("argument required: filename\n");
      return 1;
   }

   char *filename = argv[1];
   FILE *fp = fopen(filename, "r");
   if (fp == NULL) {
      printf("Unable to open '%s'\n", filename);
      return 1;
   }

   int score = get_score(fp);
   printf("score is: %d\n", score);

   fclose(fp);

   return 0;

}


int get_score(FILE *fp) {

   char line[BUF_LEN];
   int score = 0;
   while (fgets(line, BUF_LEN, fp) != NULL && strlen(line)) {
      char move_a = standardize_move(line[0]);
      char move_b = standardize_move(line[2]);
      switch (move_b) {
         case 'R': score += 1; break;
         case 'P': score += 2; break;
         case 'S': score += 3; break;
      }
      enum result result = does_b_win(move_a, move_b);
      switch (result) {
         case draw: score += 3; break;
         case win: score += 6; break;
      }
   }
   return score;
}


char standardize_move(char move) {
   if (move == 'A' || move == 'X') {
      return 'R';
   } else if (move == 'B' || move == 'Y') {
      return 'P';
   } else if (move == 'C' || move == 'Z') {
      return 'S';
   }
}


enum result does_b_win(char move_a, char move_b) {
   if (move_a == move_b)
      return draw;
   if (move_b == 'R')
      return move_a == 'S' ? win : loss;
   if (move_b == 'S')
      return move_a == 'P' ? win : loss;
   if (move_b == 'P')
      return move_a == 'R' ? win : loss;
}
